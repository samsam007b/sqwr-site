...
const GridSquare = memo(({ index }: { index: number }) => {
  ...
  const animation = useMemo(() => {
    ...
    return {
      ...
      boxShadow: startsForward ? forwardShadows : backwardShadows,
      transition: {
...
const GridLine = memo(({ lineIndex, isHorizontal }: { lineIndex: number; isHorizontal: boolean }) => {
  const animation = useMemo(() => {
    const getRandomDuration = () => 12 + Math.random() * 6;
    const getRandomDelay = () => Math.random() * 5;
  ...
  return (
    <motion.div
      className={`absolute bg-foreground/[0.03] ${isHorizontal ? 'left-0 right-0' : 'top-0 bottom-0'}`}
      style={{
